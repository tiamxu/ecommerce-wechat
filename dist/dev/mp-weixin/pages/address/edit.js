...
if (!Array) {
  const _component_up_picker = common_vendor.resolveComponent("up-picker");
  _component_up_picker();
}
  ...
  setup(__props) {
    ...
    function openCityPicker() {
      if (!form.value.province) {
        common_vendor.index.showToast({ title: "请先选择省份", icon: "none" });
        return;
      }
    ...
    async function saveAddress() {
      ...
      if (!form.value.province || !form.value.city || !form.value.address) {
        common_vendor.index.showToast({ title: "请选择完整地区", icon: "none" });
        return;
      }
      loading.value = true;
      try {
        ...
        if (isEdit.value && addressId.value) {
          await api_order.orderApi.updateAddress(addressId.value, params);
        } else {
          await api_order.orderApi.addAddress(params);
        }
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        a: form.value.receiverName,
        b: common_vendor.o(($event) => form.value.receiverName = $event.detail.value),
        c: form.value.phone,
        d: common_vendor.o(($event) => form.value.phone = $event.detail.value),
        e: form.value.province
      }, form.value.province ? {
        f: common_vendor.t(form.value.province)
      } : {}, {
        g: common_vendor.o(openProvincePicker),
        h: form.value.city
      }, form.value.city ? {
        i: common_vendor.t(form.value.city)
      } : {}, {
        j: common_vendor.o(openCityPicker),
        ...
        p: common_vendor.o(onProvinceConfirm),
        q: common_vendor.o(($event) => showProvincePicker.value = false),
        r: common_vendor.p({
          show: showProvincePicker.value,
          columns: [common_vendor.unref(provinces)],
          keyName: "label"
        }),
        s: common_vendor.o(onCityConfirm),
        t: common_vendor.o(($event) => showCityPicker.value = false),
        v: common_vendor.p({
          show: showCityPicker.value,
          columns: [cities.value],
          keyName: "label"
        }),
        w: common_vendor.n(common_vendor.unref(theme_config.THEME_CLASS))
      });
    };
  }
});
